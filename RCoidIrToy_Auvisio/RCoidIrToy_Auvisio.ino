...
#define IR_RECEIVER_PORT 5
#define STATUS_LED 4
#define SWITCH_PORT 13
...
unsigned long timer;
#define RESET_TIMEOUT 300000  //Wenn sich im AP-Modus kein Client verbindet, wird nach dieser Zeit ein Reset ausgelöst. Sinnvoll, wenn nach Stromausfall der Router noch nicht neu gestartet hat. 300000 = 5Min
...
bool WaitForConnection(int t)
{
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    delay(500);
    Serial.print(".");
    digitalWrite(STATUS_LED, counter%2==1?HIGH:LOW);
  }
  return true;
}
...
void loop()
{
  if ((WiFi.localIP().toString().equals("0.0.0.0")||WiFi.localIP().toString().equals("(IP unset)")) && WiFi.softAPgetStationNum() == 0)
  {
